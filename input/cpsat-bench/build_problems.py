...
Usage:
    python3 input/cpsat-bench/build_problems.py [flags]

Flags:
    --filter-decisive             keep only OPTIMAL / FEASIBLE rows
...
Output schema (one row per meta.jsonl entry):
    {
      "problem_sha256": "<sha>",
      "problem_filename": "<sha>.cpsat.pb",
      "cpsat_status": {"result": "OPTIMAL", "elapsed_ms": 6896.45, ...},
      "cpsat_response_stats": {"num_branches": 79884, ...},
      "cpsat_applied_params": {...},
      "features": {"num_constraints": 51623, ...},
      "applied_params_hash": "...",
...
def scan(raw_dir, *, filter_decisive=False, applied_hash_prefix=None):
    ...
    for p in metas:
        ...
        if not isinstance(d, dict):
            bad += 1
            continue
        if "problem_sha256" not in d or "problem_filename" not in d:
        ...
        if applied_hash_prefix and not str(d.get("applied_params_hash", ""))\
            ...
            continue
        if filter_decisive:
            res = (d.get("cpsat_status") or {}).get("result")
...
def main():
    ...
    ap.add_argument("--filter-decisive", action="store_true",
                    help="keep only OPTIMAL / FEASIBLE rows")
